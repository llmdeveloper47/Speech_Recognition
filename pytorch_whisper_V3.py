...
"""
# script_path = "/content/drive/MyDrive/NaturalLanguageProcessing/Transformers/Speech_Recognition/pytorch-whisper.py"

# !pip install -r "/content/drive/MyDrive/NaturalLanguageProcessing/Transformers/Speech_Recognition/requirements.txt"
...
def master_function():
  ...
  train_dataloader = DataLoader(final_audio_dataset["train"], shuffle=True, batch_size=8, collate_fn=data_collator)
  eval_dataloader = DataLoader(final_audio_dataset["validation"], batch_size=8, collate_fn=data_collator)

  optimizer = AdamW(model.parameters(), weight_decay = weight_decay, lr= learning_rate, eps = adam_epsilon)
  ...
  n_training_steps = num_train_epochs * len(train_dataloader)
  ...
  for epoch in range(num_train_epochs):
      ...
      print("training completed for " + str(epoch) + "epoch")
      model.eval()
      for step, batch in enumerate(eval_dataloader):
          ...
          ground_truth.append(references)
...
#   print("Model Saved To Directory")
  ...
  return y_pred, ground_truth, model

if __name__ == "__main__":
    torch.cuda.empty_cache()
    y_pred, ground_truth, model = master_function()